...
import { patchDetailApi } from "../../api/planningApi";
import type { RoadmapCardProps, MonthlyDetail } from "../../type/planningType";

const getCategoryIcon = (category: string) => {
  switch (category) {
    case "TECH_STACK":   return <FaLayerGroup className="text-blue-500" />;
    case "ALGORITHM":    return <FaCode className="text-blue-500" />;
    case "CERTIFICATE":  return <FaCertificate className="text-blue-500" />;
    case "KNOWLEDGE":    return <FaBook className="text-blue-500" />;
    default:             return null;
  }
};
...
const RoadmapCard = ({ month, createdAt, onPrev, onNext, isFirst, isLast }: RoadmapCardProps) => {
  ...
  const toggleMutation = useMutation({
    mutationFn: (detail: MonthlyDetail) =>
      patchDetailApi(detail.detail_id, !detail.is_completed),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["planning"] });
    },
  });

  const handleToggle = (detail: MonthlyDetail) => {
    toggleMutation.mutate(detail);
  };
  ...
  return (
    <div className="relative w-full h-60 lg:w-170 lg:h-120 bg-indigo-100/80 rounded-3xl flex flex-col dark:bg-neutral-900">
      <div className="p-4 lg:p-7">
        ...
        <IoBookSharp
          className="absolute right-10 top-2 lg:right-20 lg:top-3 text-slate-400/30 w-20 h-20 lg:w-30 lg:h-30 "
        />
          ...
          <div className="flex items-center gap-4 mb-2 lg:mb-5">
            <button
              ...
              className={clsx(
                ...
                isFirst ? "text-slate-300 cursor-not-allowed dark:text-slate-950" : "text-slate-500 dark:text-primary-blue/70 hover:text-slate-950 dark:hover:text-slate-300",
              )}
            >
              <GrPrevious/>
            </button>
            ...
            <button
              ...
              className={clsx(
                "text-xl font-semibold transition-colors",
                isLast ? "text-slate-300 cursor-not-allowed dark:text-slate-950" : "text-slate-500 dark:text-primary-blue/70 hover:text-slate-950 dark:hover:text-slate-300",
              )}
            >
              <GrNext/>
            </button>
            ...
            >
              ...
              <div className="flex flex-1 justify-between">
                <div className="px-2 lg:px-5">
                  <h3 className="text-blue-950 text-sm lg:text-xl font-bold dark:text-sub-blue/60 py-3 lg:py-0 text-left">{categoryLabel[detail.category]}</h3>
                  <p className="text-slate-500 text-xs lg:text-base mt-1 whitespace-nowrap lg:py-1 hidden lg:flex">{detail.content}</p>
                </div>
                ...
                <button
                  ...
                  className="size-10 lg:size-16 bg-background rounded-full flex items-center justify-center shrink-0 transition-transform hover:scale-105 disabled:opacity-60"
                >
                  {detail.is_completed ? (
          ...
          <div
            ...
            <div
              ...
              onClick={(e) => e.stopPropagation()}>
                ...
                <button onClick={onClose} className="text-slate-400 hover:text-slate-700 transition-colors absolute right-5 top-5 ">
