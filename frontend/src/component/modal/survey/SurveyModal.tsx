...
const SurveyModal = ({ onClose }: Props) => {
  ...
  if (step === 'repo') {
    return <RepoSelectModal onClose={() => onClose?.()} onComplete={handleRepoComplete} />;
  }
  ...
  return (
    ...
    >
      ...
      <section className="z-10 relative flex flex-col gap-4 w-full">
        ...
        {isEssay ? (
          <textarea
            ...
            className={clsx(
              ...
              'bg-card-background text-font-black',
        ...
        {errorMsg && (
          <span className="text-red-400 text-sm font-medium text-center w-full">{errorMsg}</span>
        )}
